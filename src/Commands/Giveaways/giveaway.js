    ...
    options: [
        {
            ...
            options: [
                { name: 'length', description: 'The length of the giveaway', type: ApplicationCommandOptionType.String, required: true },
                { name: 'prize', description: 'The prize being given away', type: ApplicationCommandOptionType.String, required: true },
                { name: 'winners', description: 'The number of winners', type: ApplicationCommandOptionType.Number, required: true },
                { name: 'channel', description: 'The channel to send the giveaway', type: ApplicationCommandOptionType.Channel, channel_types: [ChannelType.GuildText], required: false }
            ]
        },
        {
            name: 'pause',
            ...
            type: ApplicationCommandOptionType.Subcommand,
            options: [
                { name: 'message-id', description: 'Giveaway message id', type: ApplicationCommandOptionType.String, required: true }
            ]
        },
        {
            name: 'unpause',
    ...
    async execute(interaction, client) {
        ...
        if(sub === 'unpause') {
            await client.giveawaysManager.unpause(interaction.options.getString('message-id')).then(() => {
                successEmbed.setDescription('▶️ | The giveaway has been unpaused!');
                return interaction.reply({ embeds: [successEmbed], ephemeral: true });
            }).catch((err) => {
                errorEmbed.setDescription(`❌ | Error \n\`${err}\``);
                return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
